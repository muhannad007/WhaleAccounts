...
exports.getActiveNyxciphers = async (email) => {
  ...
  for (let i = 0; i < tickets.length; i++) {
    let nyxcipher = await Nyxcipher.findOne({
      _id: tickets[i]._id.toString(),
      nyxcipher_status: NYXCIPHER_STATUS.ACTIVE,
    })
...
exports.getClosedNyxciphers = async (email) => {
  ...
  for (let i = 0; i < tickets.length; i++) {
    let nyxcipher = await Nyxcipher.findOne({
      _id: tickets[i]._id.toString(),
      nyxcipher_status: NYXCIPHER_STATUS.CLOSED,
    })
      .populate("nyxcipher_item_id")
      .exec();
...
exports.getProfile = async (email) => {
  let user = await User.findOne({ email: email }).populate("cart_entry").exec();

  if (!user) throw new NotFoundError("User not found");
  else if (!user.verified) throw new ValidationError("User not verify");

  for (let i = 0; i < user.cart_entry.length; i++) {
    user.cart_entry[i].nyxcipher_item_id = await Item.findOne(
      user.cart_entry[i].nyxcipher_item_id
    );
  }
...
const isExistCart = async ({ nyxcipher_id, ticket_count, ticket_price }) => {
  const carts = await Cart.find({ nyxcipher_id: nyxcipher_id })
    .populate("ticket_id")
    .exec();
  console.log("===>", carts);
  for (let i = 0; i < carts.length; i++) {
    if (
      carts[i].ticket_id.ticket_count == ticket_count &&
      carts[i].ticket_id.ticket_price == ticket_price
    ) {
      return true;
    }
  }
...
exports.addMyCart = async (email, body) => {
  ...
  const nyxcipher = await Nyxcipher.findOne({
    _id: body.nyxcipher_id,
    nyxcipher_status: NYXCIPHER_STATUS.ACTIVE,
  })
    .populate("nyxcipher_item_id")
    .exec();
  ...
  const result = await isExistCart({
...
exports.deleteMyCart = async (email, id) => {
  ...
  await Ticket.deleteMany({
    _id: cart.ticket_id,
    nyxcipher_id: cart.nyxcipher_id,
  });
  await cart.delete();
...
exports.saveSponsor = async (header, body) => {
  ...
  const token = jwt.sign(
    { username, email, password, role: ROLE.SPONSOR },
    JWT_KEY,
    { expiresIn: "24h" }
  );
